...
Create a first event
  ...
  $ curl -s -X PUT http://localhost:8080/api/event \
...
Check the details of the create event
  ...
  $ curl -s http://localhost:8080/api/event/1
...
Create some competitions
  ...
  $ curl -s -X PUT http://localhost:8080/api/comp \
  > -H "Content-Type: application/json" \
...
Get the ids of competitions we created, and check their details
  ...
  $ curl -s http://localhost:8080/api/event/1/comps
  {"comps":[1,2]}

  $ curl -s http://localhost:8080/api/comp/1
  {"event":1,"name":"","kind":["Jack_and_Jill"],"category":["Novice"]}

  $ curl -s http://localhost:8080/api/comp/2
