...
function ArtefactValidCount({ artefactData, artefactDataToSubmit, isDirty }: { artefactData: HeatTargetJudgeArtefactArray, artefactDataToSubmit: HeatTargetJudgeArtefactArray, isDirty: boolean }) {
...
function generateArtefactFormObject(artefactData: HeatTargetJudgeArtefactArray) {
...
export function ArtefactFormScorer({ artefactData, dataBibs }: ArtefactFormComponentProps) {
  ...
  const currentArtefacts = validate_artefacts({ htjaArray: watch(), artefact_description: artefact_description })

  return (
    <FormProvider {...formObject}>
      <ArtefactValidCount artefactData={artefactData} artefactDataToSubmit={currentArtefacts} isDirty={isDirty} />
...
export function ArtefactFormJudge({ artefactData, dataBibs }: ArtefactFormComponentProps) {
      ...
      <form onSubmit={handleSubmitCallback} className='yan_table' >
        {artefact_description.artefact === "yan" && unique_heat_number && unique_heat_number.map((heat_number) => (
          <>
            <h2>Heat {heat_number}</h2>
            <YanArtefactFormTable
              artefactData={validatedArtefacts}
              heat_number={heat_number}
              artefactInput={false}
              dataBibs={dataBibs}
            />
        ...
        {artefact_description.artefact === "ranking" && unique_heat_number && unique_heat_number.map((heat_number) => (
          <>
            <h2>Heat {heat_number}</h2>
            <RankingArtefactFormTable
              artefactData={validatedArtefacts}
...
export function ArtefactFormRoute({id_phase, id_judge, id_competition} : ArtefactFormProps) {
  ...
  const { data: artefactData, isLoading, isSuccess, isError, error } = useGetApiPhaseIdArtefactJudgeIdJudge(id_phase, id_judge);
