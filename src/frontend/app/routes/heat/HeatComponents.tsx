...
function HeatTargetRowReadOnly({ bib_list, onDelete }: HeatTargetRowReadOnlyProps) {

    return (
        <>
            <td>
                {bib_list.map(b => (
                    <p key={b.bib}>{b.target.target_type}</p>
                ))}
            </td>
            <td>
                {bib_list.map(b => (
...
export function SingleHeatTable({ heat, phaseTargets, otherTargets, heat_number, id_phase }: SingleHeatProps) {
    ...
    return (
        <div className='bib-table-container'>
            <button type="button" className='btn bib-table-column'
                disabled={!heat_number}
...
function concatHeatsTargets(heats: HeatsArray) {
...
export function HeatsListComponent({ id_phase, panel_data, heats }: { id_phase: number, panel_data: Panel, heats: HeatsArray, dataBibs: BibList }) {
