...
and update_bib =
  Api.put
    ...
    ~to_yojson:Types.DancerIdList.to_yojson
    (fun req st (b : Types.OldBibNewBib.t) ->
       let+ id = Utils.int_param req "id" in
       match b.old_bib.competition with
       | comp_id when comp_id = id ->
         Ftw.Bib.update ~st ~competition:id ~old_bib:b.old_bib.bib ~new_bib:b.new_bib.bib;
         let dancer_list : Types.DancerIdList.t = {dancers=Types.Target.dancers b.new_bib.target} in
         Ok dancer_list
       | _ -> Error (Error.generic "Competition id do not match payload")
    )
