...
let rec routes router =
  ...
  |> Router.put "/api/dancer" create_dancer
    ...
    |> Router.patch "/api/dancer/:id/as_leader" update_division_as_leader
      ~tags:["dancer"]
      ~summary:"Update dancer division as leader"
      ~parameters:[
        Types.obj @@ Spec.make_parameter_object ()
          ~name:"id" ~in_:Path
          ~description:"Id of the queried dancer"
      ...
      ~request_body:(
        Types.obj @@ Spec.make_request_body_object ()
          ~description:"New dancer division"
          ~required:true
          ~content:[
            Spec.json,
            Spec.make_media_type_object () 
              ~schema:(Types.(ref DancerDivision.ref));
          ]
      )
      ~responses:[
        "200", Types.obj @@ Spec.make_response_object ()
    ...
    |> Router.patch "/api/dancer/:id/as_follower" update_division_as_follower
      ~tags:["dancer"; "division"]
      ~summary:"update dancer division as follower"
      ~parameters:[
        Types.obj @@ Spec.make_parameter_object ()
          ~name:"id" ~in_:Path
          ~description:"Id of the queried dancer"
  ...
  Api.get
    ...
    (fun req st ->
       ...
       let ret : Types.Dancer.t = {
        ...
        as_leader = (match (Ftw.Dancer.as_leader dancer).main with
          | Some division -> division
          | None -> Novice
        );
        as_follower = (match (Ftw.Dancer.as_follower dancer).main with
          | Some division -> division
          | None -> Novice
        );
       } in
...
and create_dancer =
  Api.put
    ...
    (fun _req st (dancer : Types.Dancer.t) ->
        let as_leader : Ftw.Divisions.t = {main = Some dancer.as_leader} in
        let as_follower : Ftw.Divisions.t = {main = Some dancer.as_follower} in
        let id_dancer = Ftw.Dancer.add st dancer.birthday 
  ...
  ~of_yojson:Types.DancerDivision.of_yojson
    ...
    (
      fun req st dancer_division ->
        let+ id_dancer = Utils.int_param req "id" in
        let (as_follower : Ftw.Divisions.t) = 
          {main = Some dancer_division} in
        let id = Ftw.Dancer.update_follower_division st 
...
and update_division_as_leader = 
  Api.patch
    ...
    (
      fun req st dancer_division ->
        let+ id_dancer = Utils.int_param req "id" in
        let (as_leader : Ftw.Divisions.t) = 
          {main = Some dancer_division} in
        let id = Ftw.Dancer.update_leader_division st 
          id_dancer as_leader
        in
        Ok id
    )
